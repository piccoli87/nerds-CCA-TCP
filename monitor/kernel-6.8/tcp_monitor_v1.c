...
static void update_metrics(const struct sock *sk)
{
    ...
    if (!tm) { spin_unlock(&metrics_lock); return; }
...
static int proc_show(struct seq_file *m, void *v)
{
    struct tcp_metrics *tm;
    unsigned bkt;
    seq_printf(m, "SADDR       DADDR       SPORT DPORT CWND  SRTT  RTTVAR RET  SNDWND RCVWND\n");
    spin_lock(&metrics_lock);
    hash_for_each(tcp_metrics_hash, bkt, tm, __node) {
        seq_printf(m, "%pI4 %pI4 %5u %5u %4u %6u %7u %4u %7u %7u\n",
...
static int __init tcp_monitor_init(void)
{
    ...
    if (ret) { unregister_kprobe(&kp_retrans); return ret; }
    ret = register_kprobe(&kp_ack);
    if (ret) { unregister_kprobe(&kp_retrans); unregister_kprobe(&kp_xmit); return ret; }
